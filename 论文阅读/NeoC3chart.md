...
相较于以传统数据库与服务器搭建起的时间银行管理系统，本作品引入了基于PoS权益共识机制区块链技术，构建拥有联盟环境的时间银行联盟链，以此为基础构建起了全新的时间银行管理系统，本作品最大的优势在于利用智能合约技术实现了时间币交易的自动实现与交易完成自动广播，避免了人工记账导致的数据错误问题，得益于数字签名技术，所有的交易信息将被安全、不可修改的方式存储在区块链中，即使某一结点被攻击导致部分数据被篡改，系统依旧可以通过未陷落结点中的数据对区块链的错误数据进行修正，区块链中的所有结点将可以访问其中链上的所有数据，这不仅降低了审查难度，也对新节点的加入降低了难度，只需同步P2P网络中的数据即可进行正常的运行。
